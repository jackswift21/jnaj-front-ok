...
export class YoutubeApiService {
  ...
  constructor(options: any, private authService?: Authorization) {
    this.resetConfig();
    if (authService) {this.authorize = true;}
    if (options) {
  ...
  hasToken(): boolean {return this.authService && this.authService.accessToken.length > 0;}
  resetConfig() {
  ...
  list(id) {
    if (this.idKey) {this.config.set(this.idKey, id);}
    this.isSearching = true;
    let options: RequestOptionsArgs = {
      search: this.config,
      headers: this.createHeaders()};
  ...
  createHeaders() {
    ...
    if (accessToken && this.authorize) {headersOptions['authorization'] = `Bearer ${accessToken}`;}
