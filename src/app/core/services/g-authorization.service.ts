...
export class GoogleAuthorization {
  ...
  constructor(
    ...
    public http: Http){this.loadAuth();}
  loadAuth(){
    this.gapiLoader
      .load('auth2')
  ...
  authorize(){
    if (this._googleAuth){console.log('signedIn?',this._googleAuth.isSignedIn.get());}
    const authOptions = {
  ...
  private hasAccessToken (googleAuth: gapi.auth2.GoogleAuth): boolean {
    return googleAuth && 
      googleAuth.currentUser.get().getAuthResponse().hasOwnProperty('access_token');}
  ...
  signIn(){
    ...
    if (this._googleAuth) {
      Observable.fromPromise(this._googleAuth.signIn(signOptions)).subscribe(
          (response:any) => this.handleSuccessLogin(response), 
  ...
  handleSuccessLogin(googleUser: gapi.auth2.GoogleUser) {
    ...
    this.store.dispatch(this.userProfileActions.updateAccessToken(token));
    this.store.dispatch(this.userProfileActions.convertProfile(profile));
  ...
  startTimerToNextAuth(timeInMs: number): Subscription {
    return Observable.timer(timeInMs)
      ...
      .subscribe((googleUser: gapi.auth2.GoogleUser) => {
        this.zone.run(() => this.handleSuccessLogin(googleUser));});}
  handleFailedLogin(response) {console.log('FAILED TO LOGIN:', response);}
  isSignIn(){return this._googleAuth && this._googleAuth.isSignedIn.get();}
  signOut(){
    this.disposeAutoSignIn();
