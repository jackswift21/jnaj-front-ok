...
import { NowPlaylistInterface } from '../models/now-playlist.model';
import { NowPlaylistActions } from '../actions/now-playlist.actions';
...
export class NowPlaylistService {
  ...
  removeVideo(media) {this.store.dispatch(this.nowPlaylistActions.removeVideo(media));}
  selectVideo(media) {this.store.dispatch({ type: NowPlaylistActions.SELECT, payload: media });}
  updateFilter(filter: string) {
    this.store.dispatch({ type: NowPlaylistActions.FILTER_CHANGE, payload: filter });}
  clearPlaylist() {this.store.dispatch({ type: NowPlaylistActions.REMOVE_ALL });}
  selectNextIndex() {this.store.dispatch(this.nowPlaylistActions.selectNext());}
  selectPreviousIndex() {this.store.dispatch(this.nowPlaylistActions.selectPrevious());}
  trackEnded() {this.store.dispatch(this.nowPlaylistActions.mediaEnded());}
  getCurrent() {
    let media;
  ...
  toggleRepeat() {this.store.dispatch(this.nowPlaylistActions.toggleRepeat());}
  seekToTrack(trackEvent) {this.store.dispatch(this.nowPlaylistActions.seekTo(trackEvent));}*/
}
