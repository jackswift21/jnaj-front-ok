...
export class YoutubeDataApi {
  ...
  private get _apiUrl () {return `${this._apiPrefixUrl}/${this._apiVersion}`;}
  ...
  delete (api: string, options) {return this._request(api);}
  insert (api: string, options) {return this.http.post(this.getApi(api), {});}
  update (api) {return this._request(api);}
  ...
  private getApi(api: string): string {return `${this._apiUrl}/${api}`;}
  private mergeParams (source, target: URLSearchParams) {
    Object.keys(source).forEach(param => target.set(param, source[param]));}*/
}
