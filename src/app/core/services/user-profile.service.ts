...
import { GoogleBasicProfile } from '../models/user-profile.model';
...
export class UserProfile {
  ...
  constructor(
    ...
    this.playlistInfo = new YoutubeApiService({
      ...
      http: this.http,
      idKey: 'playlistId',
      config: {mine: 'true'}}, authorization);
    ...
    this.playlists = new YoutubeApiService({
      ...
      config: {
        mine: 'true',
        part: 'snippet,id,contentDetails'},}, authorization);
    this.playlistApi = new YoutubeApiService({
      url: 'https://www.googleapis.com/youtube/v3/playlists',
      http: this.http,
      idKey: 'id',
      config: {part: 'snippet,id,contentDetails'},}, authorization);}
  ...
  fetchPlaylist(playlistId: string) {return this.playlistApi.list(playlistId);}
