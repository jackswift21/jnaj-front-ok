...
import { NowPlaylistInterface, initialState } from './now-playlist.model';
...
export function nowPlaylist(state: NowPlaylistInterface = initialState, action: Action): NowPlaylistInterface {
  switch (action.type) {
    case NowPlaylistActions.SELECT:return Object.assign({}, state, { selectedId: action.payload.id });
    ...
    case NowPlaylistActions.REMOVE:
      return Object.assign({}, state, { 
        videos: removeMedia(state.videos, action.payload) });
    ...
    case NowPlaylistActions.MEDIA_ENDED:return selectNextOrPreviousTrack(state, state.filter);
    case NowPlaylistActions.TOGGLE_REPEAT: {return {...state,repeat:!state.repeat};};
    default:return state;}};

export const nowPlaylistRegister = {reducer: { nowPlaylist }, actions: NowPlaylistActions};
