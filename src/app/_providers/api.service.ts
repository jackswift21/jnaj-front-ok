...
export class ApiService {
  url = environment.localAPI;//api_jnaj;
  constructor(private http:Http,private win:AppWindowService){}
  ...
  private setHeaders(path):Headers{
    ...
    this.win['jnajTkn']?(headersConfig['Authorization'] = `Token ${this.win['jnajTkn']}`):null;
    return new Headers(headersConfig);}
  private formatErrors(error:any){
    let e = error instanceof Response?error.json():error;
    //console.log(e.name?);
    return Observable.throw(e);}
  locateIP(ip){return this.get("https://www.iplocation.net/?query="+ip).map(res => res.json())}
